...
CREATE TABLE IF NOT EXISTS professional_role_categories (
...
CREATE TABLE IF NOT EXISTS professional_roles (
    id INTEGER PRIMARY KEY,
    name VARCHAR(255) NOT NULL,
    category_id INTEGER REFERENCES professional_role_categories(id),
...
CREATE TABLE IF NOT EXISTS vacancies (
    ...
    -- Адрес (денормализован для упрощения)
    ...
    -- Режимы работы (сохраняем как текст через запятую)
    working_days TEXT,
...
CREATE TABLE IF NOT EXISTS vacancy_professional_roles (
    ...
    FOREIGN KEY (vacancy_id, vacancy_published_at, vacancy_created_at, vacancy_parsed_at) 
        REFERENCES vacancies(id, published_at, created_at, parsed_at) ON DELETE CASCADE
);
